...
csv_filename = f"experiments/rnn_experiment_maxpooling.csv"
with open(csv_filename, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["RNN_Type", "BestEpoch", "TrainAcc", "ValAcc", "EpochsRun", "TrainTime_sec"])

    for rnn_type in rnn_types:
        logger.info(f"\n--- Training with RNN type: {rnn_type.upper()} ---")
        model = RNNClassifier(
        ...
        writer.writerow([rnn_type, best_epoch, round(best_train_acc, 4), round(best_val_acc, 4), epoch + 1, train_time])
        logger.info(f"Finished {rnn_type.upper()} | Best Val Acc: {best_val_acc:.4f} | Time: {train_time} sec\n")
