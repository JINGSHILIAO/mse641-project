...
class Dataset(Dataset):
    def __init__(self, filepath, tokenizer_name='bert-base-uncased', max_len=128, is_test=False):
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                max_text = title.strip()
                for para in paragraphs:
                    if len(max_text.split()) > 400:
                        break
                    max_text += " " + para.strip()
...
def get_dataloader(filepath, batch_size=32, tokenizer_name='bert-base-uncased', max_len=128, is_test=False):
    dataset = Dataset(filepath, tokenizer_name, max_len, is_test)
